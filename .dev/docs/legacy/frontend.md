...
**Что можно**:
- ✅ Импортировать **ТОЛЬКО types/schemas** из других entities (через явный barrel)
...
// ❌ entities → другая entity
import { useEdges } from '@/entities/edge' // из node!
...
- **Минимальная связанность** - entities остаются независимыми
- **Четкий контракт** - barrel export явно контролирует экспортируемые типы
- **Легко рефакторить** - внутренняя логика entity не влияет на другие
- **Типобезопасность** - TypeScript проверит использование типов

### Source of Truth

Domain типы живут в source entity:

```tsx
// ✅ ПРАВИЛЬНО
entities/node/node.schema.ts:
export type NodeId = string  // Source of truth

entities/user/user.schema.ts:
export type UserId = string  // Source of truth

entities/edge/model/edge.schema.ts:
import type { NodeId } from '@/entities/node'  // Используем от source

// ❌ ПЛОХО - дублирование типов
shared/types/common.ts:
export type NodeId = string  // Не делайте так!
```
...
4. **Cross-entity imports**: ТОЛЬКО types/schemas через явный barrel
