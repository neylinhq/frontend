    ...
    <div className='flex h-11 items-center justify-between bg-muted/30 px-2.5'>
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align='end' className='w-40'>
          ...
          <DropdownMenuItem onClick={onCloseOthers} disabled={sessions.length <= 1}>
            {t('ai.chat.closeOthers', 'Close other tabs')}
          </DropdownMenuItem>
          <DropdownMenuItem onClick={onCloseAll} disabled={sessions.length === 0}>
            {t('ai.chat.closeAll', 'Close all tabs')}
          </DropdownMenuItem>
          <DropdownMenuSeparator />
          <DropdownMenuItem disabled>{t('ai.chat.exportChat', 'Export chat')}</DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>
    </div>
  )
