...
export const useViewport = (options: UseViewportOptions = {}) => {
  const { initialViewport, minZoom = 0.1, maxZoom = 4, onViewportChange } = options
  ...
  const fitToBounds = useCallback(
    (minX: number, minY: number, maxX: number, maxY: number, padding = 0.1) => {
      setViewport(v => {
...
export const useGraphEngine = ({ canvas, onPositionsUpdate }: UseGraphEngineOptions) => {
  ...
  const startAnimatedLayout = useCallback(
...
export const useLegacyGraphEngine = (
  _options: UseLegacyGraphEngineOptions = {}
