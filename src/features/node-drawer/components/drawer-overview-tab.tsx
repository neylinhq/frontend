...
import { getComplexityColor, getNodeBorderColor, getNodeIcon } from '@/features/graph-visualization/lib/get-node-style'
      ...
      <div>
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs text-muted-foreground w-20">{t('nodeDrawer.overview.type')}:</span>
          ...
          {node.metadata.complexity && (
            <div className="flex items-center gap-2">
              <span className="text-xs text-muted-foreground w-20">{t('nodeDrawer.overview.complexity')}:</span>
              <Badge
                variant="secondary"
                className={cn('text-xs pointer-events-none', getComplexityColor(node.metadata.complexity))}
              >
                {node.metadata.complexity}
              </Badge>
            </div>
          ...
          {node.metadata.confidence !== undefined && (
            <div className="flex items-center gap-2">
              <span className="text-xs text-muted-foreground w-20">{t('nodeDrawer.overview.confidence')}:</span>
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-medium">{Math.round(node.metadata.confidence * 100)}%</span>
                </div>
              </div>
            </div>
          )}
          ...
          {node.metadata.tags && node.metadata.tags.length > 0 && (
            <div className="flex items-start gap-2">
              <span className="text-xs text-muted-foreground w-20 pt-1">{t('nodeDrawer.overview.tags')}:</span>
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs">{new Date(node.createdAt).toLocaleDateString(i18n.language)}</span>
          </div>

          <div className="flex items-center gap-2">
            <span className="text-xs text-muted-foreground w-20">{t('common.updated')}:</span>
            <span className="text-xs">{new Date(node.updatedAt).toLocaleDateString(i18n.language)}</span>
          </div>
