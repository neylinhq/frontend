...
const validateComponentsSegment = (
  ...
  // Check file types - allow .tsx, .types.ts, .constants.ts, .schema.ts, .utils.ts, .hooks.ts, .module.css, .test.ts
  for (const file of files) {
    if (file.startsWith('.')) continue

    const allowedInComponents = ['.tsx', '.types.ts', '.constants.ts', '.schema.ts', '.utils.ts', '.hooks.ts', '.module.css', '.test.ts', '.test.tsx']
    ...
    if (!hasAllowedExt) {
      errors.push({
        ...
        message: `Only .tsx, .types.ts, .constants.ts, .schema.ts, .utils.ts, .hooks.ts, .module.css allowed in /components, found '${file}'`,
        severity: 'error'
      })
    }
  }
...
const validateModelSegment = (
  ...
  for (const file of files) {
    ...
    // Check domain is either module name or component name
    const isModuleDomain = domain === moduleName
    const isComponentDomain = componentNames.some((c) => c.replace('.tsx', '') === domain)
    ...
    if (!isModuleDomain && !isComponentDomain) {
      ...
      errors.push({
        path: `${relPath}/${file}`,
        rule: 'model-naming',
        message: `Domain '${domain}' not found. Valid: ${validDomains.join(', ')}`,
        severity: 'warning'
      })
    }
  }
